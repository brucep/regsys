...
function regsys_report_index_visualization()
{
	...
	$events   = $database->query('SELECT event_id, name, date_paypal_prereg_end FROM regsys_events WHERE visualization = 1 ORDER BY name ASC')->fetchAll(PDO::FETCH_OBJ);
	...
	$dates = $database->query('SELECT MIN(DISTINCT FROM_UNIXTIME(date_registered, "%%m-%%d")) AS start, MAX(DISTINCT FROM_UNIXTIME(date_registered, "%%m-%%d")) AS end FROM regsys_dancers WHERE date_registered > 1 AND event_id IN (SELECT event_id FROM regsys_events WHERE visualization = 1)')->fetch(PDO::FETCH_OBJ);
	...
	foreach ($dates as $date) {
		...
		foreach ($events as $event) {
			...
			elseif ($date > date('m-d', $event->date_paypal_prereg_end)) {
				...
				$count = (int) $database->query('SELECT COUNT(dancer_id) FROM regsys_dancers WHERE FROM_UNIXTIME(date_registered, "%%m-%%d") <= ? AND event_id = ?', array($date, $event->event_id))->fetchColumn();
