...
<div class="wrap" id="nsevent">
	...
	<p><?php printf('<strong>%s (%d)</strong>: <em>%s</em>', $dancer->get_name(), $dancer->get_id(), $dancer->housing_type); ?></p>
...
<div class="wrap" id="nsevent">
	...
	<form action="<?php bloginfo('wpurl'); ?>/wp-admin/admin.php?page=nsevent&amp;event_id=<?php echo $event->get_id(); ?>&amp;request=housing-delete&amp;parameter=<?php echo $_GET['parameter'] ?>" method="post">
		<p><?php printf('<strong>%s (%d)</strong>: <em>%s</em>', $dancer->get_name(), $dancer->get_id(), $dancer->housing_type); ?></p>
