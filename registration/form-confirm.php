<?php if (!get_post_meta($post->ID, 'nsevent_registration_form', true)) { get_header(); } ?>
				...
				<div id="nsevent-registration-form-confirm" <?php post_class('nsevent-registration-form'); ?>>
					...
					<form action="<?php echo get_permalink(); ?>" method="post" <form action="<?php echo get_permalink(); ?>" method="post" class="<?php echo $early_bird_class; if ($vip) { echo ' vip'; } ?>">>
						...
						<div class="field"><span class="label"><?php _e('Email Address', 'nsevent'); ?>:</span> <?php echo esc_html($dancer->email); ?></div>
						<?php NSEvent_FormInput::hidden('email'); echo "\n"; ?>
						
						<div class="field"><span class="label"><?php _e('Position', 'nsevent'); ?>:</span> <?php echo esc_html($dancer->position()); ?></div>
						<?php NSEvent_FormInput::hidden('position'); echo "\n"; ?>
<?php if ($event->levels): ?>
						<div class="field"><span class="label"><?php _e('Level', 'nsevent'); ?>:</span> <?php echo esc_html($dancer->level()); ?></div>
						<?php NSEvent_FormInput::hidden('level'); echo "\n"; ?>
<?php endif; ?>

<?php if ($event->has_discounts() and !$vip): ?>
<?php 	if ($_POST['discount'] != 0): ?>
						<div class="field"><?php _e('&#10004;&nbsp;', 'nsevent'); echo esc_html($event->{'discount'.$_POST['discount']}); ?></div>
<?php 	endif; ?>
						<?php NSEvent_FormInput::hidden('discount'); echo "\n"; ?>
<?php endif; ?>			
...
<?php foreach (NSEvent::$validated_items as $key => $item): ?>
						<div class="field"><?php _e('&#10004;&nbsp;', 'nsevent'); echo esc_html($item->name); if (isset($_POST['item_meta'][$key])) printf(' <span class="item-meta">(%s)</span>', esc_html(ucfirst($_POST['item_meta'][$key])));?></div>
